...
<picture>
  ...
  <img alt="ctxtray のパネル。5時間枠と週間枠、その下に 3 つのセッションのコンテキスト使用量" src="docs/images/ctxtray-hud-ja-light.png" width="352">
</picture>
...
閾値を超えた値だけが黄、さらに赤に変わります。出し方は 2 通りあり、設定で切り替えられます。
...
アイコンにコンテキストを出す場合は、**最も圧縮に近いセッション**を選びます。
...
```jsonc
{
  "thresholds": {
    ...
    // 圧縮点は観測によって較正する前提の値なので、「85% で赤」と絶対値で固定すると、
    // 実際の圧縮点が 80% だと分かったときに「圧縮された後に赤くなる」からです。
...
- **圧縮点は暫定値（0.92）です。** 実際の圧縮を観測して較正する仕組みは、
  設計済みですが未実装です。コンテキストの色と通知はこの値に依存します。
