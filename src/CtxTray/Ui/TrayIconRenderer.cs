...
namespace CtxTray.Ui
{
    ...
    ///
    /// ★ パネルが注意・危険に重ねる縞（Ui/LevelMark.cs）は、ここには入れない。
    ///   16〜24px では、はっきり見えるようにすると今度はうるさくなる。
    ///   実寸で 3 案を見比べた上での利用者の決定（2026-09-20）。揃えに行かないこと。
    /// </summary>
    internal static class TrayIconRenderer
    {
        ...
        private static void DrawMarked(Graphics g, int side, TrayGauge gauge, string style, Theme theme)
        {
            ...
            DrawBar(g, new RectangleF(barX, barY, side - barX * 2, barH), gauge, theme, barH / 2f);
